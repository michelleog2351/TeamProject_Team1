...
function getJsonData() {
  $.getJSON(`http://localhost:3000/booking`, function (data) {
    $.each(data, function (i, value) {
      $(`#tbody`).append(
        `<tr>
				...
				<td><button type="button" class="updateButton btn btn-secondary" value="${value.BookingID}">Update</button></td>
    ...
    $(".deleteButton").click(function (e) {
      let ID = e.target.value;
      $.post(`http://localhost:3000/deleteBooking/${ID}`, { BookingID: ID }).done(
        function () {
